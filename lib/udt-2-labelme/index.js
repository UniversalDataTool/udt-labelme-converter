...
const zip = async(folderName) =>{
  const zipAFolder = require('../utils/zip-a-folder/index')
  zipAFolder(folderName)
}

const main = async ({inputFile, outputFolder, willZipp = false}) => {
  let imageWidth
  let imageHeight
  ...
  for (const sample of udtJSON["samples"]) {
    try {
      ...
      const fileName = sample.cutomId ? sample.customId.split("/").slice(-1)[0] : sample.imageUrl.split("/").slice(-1)[0]
      ...
      console.log(`${(udtJSON["samples"].indexOf(sample)) + 1}/${udtJSON["samples"].length} files converted: ${fileNameWithoutExtension}`)
    } catch (e) {
  ...
  if(willZipp){
    console.log("Zipping is started")
    setTimeout(() => {
      zip(outputFolder)      
    }, 2000)
  }else{
    console.log(`You can find you converted files in ${outputFolder} folder`)
  }
}
